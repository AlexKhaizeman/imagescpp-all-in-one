...
// ширина окна; 
// высота окна
unsigned short **channelSegmentation3(string path, string info, unsigned short **channel_R, int width, int height, int bi, int points_number, int frameWidth, int frameHeight){
	ofstream f;
	...
	for ( ssize_t row = 0; row < height ; row++ ){
		for ( ssize_t column = 0; column < width ; column++ ){
			if (pi_average[row][column] > last_min_number_2*accuracy){
	...
	//step 2. Change the window-size.
	f<<"Адаптируем размер окна в зависимости от вероятности перехода для каждого пикселя"<<endl;
	//one more hist, one more result array, one more output picture.
	//pi sensetive window parameters:
	int 
		frameWidth_2 = 3, frameHeight_2 = 3, frameSquare_2 = 9,
		frameWidthHalf_2 = 1, frameHeightHalf_2 = 1, 
		//small window on the edges:
		frameWidthHalfLocal_2 = 1, frameHeightHalfLocal_2 = 1, frameSquareLocal_2 = 1;
	//another output image:
	Image dst2_image(Geometry(width, height), ColorRGB(MaxRGB, MaxRGB, MaxRGB));
	//
	dst2_image.modifyImage();
	Pixels dst2_view(dst2_image);
	//another array for it:
	unsigned short **channel_R_2 = new unsigned short*[height]; //Declare a COPY model of an image for current BI
	for(int i = 0; i < height; i++) {
		channel_R_2[i] = new unsigned short[width];
	}	
	//обнуление
	for ( ssize_t row = 0; row < height ; row++ ){
		for ( ssize_t column = 0; column < width ; column++ ){
			channel_R_2[row][column] = 0;
		}
	}
	//another histogramm:
	histHeight = 600; histWidth = 1800;
	//чтобы рисовать гистограммы
	Image hist_image2(Geometry(histWidth, histHeight), Color(MaxRGB, MaxRGB, MaxRGB, 0));
	histHeight -= 30;
	histWidth -= 5;
	hist_image2.strokeColor("olive"); // Outline color 
	hist_image2.fillColor("green"); // Fill color 
	hist_image2.strokeWidth(1);
	//axes
	histHeight-=25;
	drawList.push_back(DrawableLine(5, histHeight, histWidth, histHeight)); //X ось
	drawList.push_back(DrawableLine(5, 0, 5, histHeight)); // Y ось
	...
	histHeight+=20;
	//draw and clear
	hist_image2.draw(drawList);
	drawList.clear();
	//new points, new pi-s, new minims... etc. so far.
	double *points_2 = new double[points_number];
	int *vicinities_2 = new int[points_number];
	int *point_index_2 = new int[points_number]; //номера точек для сортировки окрестностей
	bool *minims_2 = new bool[points_number];
	int minims_counter_2 = 0;
	int minims_flag_2 = 0; //это флаг, на каком минимуме мы сейчас находимся, когда идём по гистограмме (чтобы в соответствующий цвет красить)
	//pi
	double **pi_2 = new double*[height]; // (!) pi - для центра окна - среднее по окну (типа симметричные пи1-пи2)
	double **pi_average_2 = new double*[height]; // среднее PI
	double **PI_2 = new double*[height]; //PI = pi1*pi2/pi3
	for(int i = 0; i < height; i++) {
		pi_2[i] = new double[width];
		PI_2[i] = new double[width];
		pi_average_2[i] = new double[width]; //PI average
	}		
	//работа непосредственно - копия с 111 по 398 строки. не умею по-другому
	// вероятность pi1 pi2 - работает не чини
	for ( ssize_t row = 0; row < height ; row++ ){
		for ( ssize_t column = 0; column < width ; column++ ){
			if (column > frameWidthHalf_2 && row > frameHeightHalf_2 && column < (width - frameWidthHalf_2) && row < (height - frameHeightHalf_2)){
				//make a 1d copy
				index = 0;
				//frame circle
				for (ssize_t frameRow = (row - frameHeightHalf_2); frameRow <= (row + frameHeightHalf_2); frameRow++){
					for (ssize_t frameColumn = (column - frameWidthHalf_2); frameColumn <= (column + frameWidthHalf_2); frameColumn++){                    	
						biElementsFrame[index] = channel_R_[frameRow][frameColumn];
						index++;
					}
				}								
				...
				for (int j = 1; j < frameSquare; j++){
					if (biElementsFrame[j - 1] == biElementsFrame[j])
						...
						{ sequences++; }
					element++;
					pi_2[row][column] -= (double) sequences / (double) element;
				}                
				pi_2[row][column] = (double) pi_2[row][column]/ (double) frameSquare;      
			}
			else{
				if (column <= frameWidthHalf_2){
					frameWidthHalfLocal_2 = column;
				}
				if (column >= (width - frameWidthHalf_2)){
					frameWidthHalfLocal_2 = width - column - 1;
				}
				if (row <= frameHeightHalf_2){
					frameHeightHalfLocal_2 = row;
				}
				if (row >= (height - frameHeightHalf_2)){
					frameHeightHalfLocal_2 = height - row - 1;
				}				
				frameSquareLocal = (2 * frameHeightHalfLocal_2 + 1) * (2 * frameWidthHalfLocal_2 + 1);
				//make a 1d copy
				index = 0; 
				//frame circle
				for (ssize_t frameRow = (row - frameHeightHalfLocal_2); frameRow <= (row + frameHeightHalfLocal_2); frameRow++){
					for (ssize_t frameColumn = (column - frameWidthHalfLocal_2); frameColumn <= (column + frameWidthHalfLocal_2); frameColumn++){                    	
						biElementsFrame[index] = channel_R[frameRow][frameColumn];
						index++;
					}
				}
				...
				element = 1;
				sequences = 1;
				pi_2[row][column] = frameSquareLocal;
				for (int j = 1; j < frameSquareLocal; j++){
					if (biElementsFrame[j - 1] == biElementsFrame[j])
						{ element++; }
					else
				...
				pi_2[row][column] = (double) pi_2[row][column]/ (double) frameSquareLocal;                  	
			}
		}
	}
	// dump("pi1 and pi2 counted...");
	for ( ssize_t row = 0; row < height ; row++ ){
		for ( ssize_t column = 0; column < width ; column++ ){								
			if (pi_2[row][column] < 0){
				f<<"warning: pi<0: " + to_string(pi_2[row][column])+" row: " + to_string(row) + " column: " + to_string(column)<<endl;
				f<<" " + to_string(element) + " / " + to_string(sequences) + " / " + to_string(frameSquare) + " = " + to_string(element / sequences / frameSquare)<<endl;
				pi_2[row][column] = 0.001;
			}
			if (pi_2[row][column] > 1.0){
				f<<"warning: pi>1: " + to_string(pi_2[row][column])+" row: " + to_string(row) + " column: " + to_string(column)<<endl;
				f<<" " + to_string(element) + " / " + to_string(sequences) + " / " + to_string(frameSquare) + " = " + to_string(element / sequences / frameSquare)<<endl;
				pi_2[row][column] = 0.999;
			}						
		}
	}		
	// dump("pi1 and pi2 checked ...");
	...
	for ( ssize_t row = 1; row < height ; row++ ){
		for ( ssize_t column = 1; column < width ; column++ ){
			...
			//0x0 1x1
				...
				//000 111
				if (channel_R_[row][column] == channel_R_[row][column-1]){
					PI_2[row][column] = (double) (alpha * alpha / pi3); 
				}
				//010 101
				else{					
					PI_2[row][column] = (double) (aalpha * aalpha / pi3); 
				}
			}
		}
	}		
	...
	for ( ssize_t row = 0; row < height ; row++ )//вероятность pi1 pi2
	{
		for ( ssize_t column = 0; column < width ; column++ )
		{
			pi_average_2[row][column] = 0;
			if (column > frameWidthHalf_2 && row > frameHeightHalf_2 && column < (width - frameWidthHalf_2) && row < (height - frameHeightHalf_2)){
				for ( ssize_t frameRow = row - frameHeightHalf_2; frameRow <= row + frameHeightHalf_2; frameRow++ ){
					for ( ssize_t frameColumn = column - frameWidthHalf_2; frameColumn <= column + frameWidthHalf_2; frameColumn++ ){
						pi_average_2[row][column] += (double) PI_2[frameRow][frameColumn];
					}
				}
				pi_average_2[row][column] = (double) pi_average_2[row][column] / (double) frameSquare;				                        	
			}
			else{
				if (column <= frameWidthHalf_2){
					frameWidthHalfLocal_2 = column;
				...
				pi_average_2[row][column] = (double) pi_average_2[row][column] / (double) frameSquareLocal;				                        	
			}
		}
	}	
	// dump("Pi3-s frame average counted ...");
	for (int j = 0; j < points_number; j++)
		points_2[j] = 0;
	// dump("created points_2 ...");
	//если неправильно вычислить пи - будет сегфолт, что логично. смотри выше на глюки. - всё поправлено. 20161023. Ну, давно нормально работает.
	for ( ssize_t row = 0; row < height ; row++ ){
		for ( ssize_t column = 0; column < width ; column++ ){
			pi_average_2[row][column] = Round(pi_average_2[row][column], accuracy);
			if (pi_average_2[row][column] <= 0){
				pi_average_2[row][column] = 0.001;
			}
			if (pi_average_2[row][column] >= 1.0){
				pi_average_2[row][column] = 0.999;
			}				
		}
	}
	// dump("pi rounded ...");
	...
	for ( ssize_t row = 0; row < height ; row++ ){
		for ( ssize_t column = 0; column < width ; column++ ){
			current_points_number = (int)(pi_average_2[row][column] / accuracy);
			if (current_points_number >= points_number){
				f<<"warning more: in " + to_string(current_points_number)+" pi is: " + to_string(pi_average_2[row][column])<<endl;
				current_points_number = points_number - 1;
			}
			if (current_points_number<0){
				f<<"warning zero: in " + to_string(current_points_number)+" pi is: " + to_string(pi_average_2[row][column])<<endl;				
				current_points_number = 0;
			}
			points_2[current_points_number]++;					
		}
	}
	// dump("counted histogramm points_2 ...");
	//нахождение наивысшей точки (максимального количества), нормировка по наивысшей точке    
	max_point = 0;
	for (int j = 0; j < points_number; j++)
		if (max_point < points_2[j])
			max_point = points_2[j];
	//прорисовка на гистограмме
	hist_image.strokeColor("black"); // Outline color 		   
	histHeight -= 25;
	histWidth -= 5;
	for (int j = 0; j < points_number; j++){
		points_2[j] /= max_point;
		drawList.push_back(DrawableLine(
			(histWidth * j / points_number) + 5,
			(int)(histHeight - histHeight * points_2[j]),
			(histWidth * j / points_number) + 5,
			(int)histHeight));
	}
	for (int j = 1; j <= points_number; j++){
		drawList.push_back(DrawableLine(
			(histWidth * (j-1) / points_number) + 5,
			(int)(histHeight - histHeight * points_2[j-1]),
			(histWidth * j / points_number) + 5,
			(int)(histHeight - histHeight * points_2[j])));				
	}
	histHeight += 25;
	...
	minims_2[0] = true; // чтобы цветов было на один больше чем минимумов, введём минимум в нуле, чтобы привязать к нему первый цвет
	//вычисляем окрестности, в которых данная точка является минимумом
	vicinity = 1;
	a_vicinity_flag = true;
	for (int j = points_number/2; j < points_number - 1; j++){
		vicinity = 1;
		while(a_vicinity_flag){
			if (!((points_2[j]<=points_2[j-vicinity] & points_2[j]<points_2[j+vicinity])||(points_2[j]<points_2[j-vicinity] & points_2[j]<=points_2[j+vicinity]))){
				a_vicinity_flag = false;
			}
			else{
				vicinity++;
		...
		vicinities_2[j] = vicinity-1;
		a_vicinity_flag = true;	// f<<to_string(vicinities_2[j]) + " "<<endl;
	}
	// dump("got vicinities_2 ...");
	//попробуем просто отсортировать
	for (std::size_t idx_i = 0; idx_i < points_number - 1; idx_i++)
	{
		for (std::size_t idx_j = 0; idx_j < points_number - idx_i - 1; idx_j++)
		{
			if (vicinities_2[idx_j + 1] > vicinities_2[idx_j])
			{
				std::swap(vicinities_2[idx_j], vicinities_2[idx_j + 1]);
				std::swap(point_index_2[idx_j], point_index_2[idx_j + 1]);                
			}
		}
	}
	// dump("sorted vicinities_2 ...");
	drawList.push_back(DrawableStrokeColor(Color("red")));
	minims_counter_2 = 0;
	double margin_2 = pow(vicinities_2[0], 0.7); //допустим так пока
	for (int ii=0; ii<20; ii++){
		if ((double) vicinities_2[ii] > margin_2)
			minims_counter_2++;
	}
	for (int ii=0; ii<minims_counter_2; ii++){
		minims_2[point_index_2[ii]] = true;
		f<<"Есть минимум в точке " + to_string(point_index_2[ii]) + " равный " + to_string(point_index_2[ii] * accuracy) + " окрестность " + to_string(vicinities_2[ii])<<endl;
		drawList.push_back(DrawableText(point_index_2[ii] * accuracy * histWidth, histHeight, to_string(point_index_2[ii] * accuracy)));
		drawList.push_back(DrawableLine(
			(histWidth * point_index_2[ii] * accuracy) + 5,
			5,
			(histWidth * point_index_2[ii] * accuracy) + 5,
			(int)histHeight-10));    	
	}
	...
	//далее сегментация
	//normal colours: step*255/counter
	minims_counter_2++; //нам нужен ещё один цвет для самого крайнего сегмента. там будет зависимость от яркости
	unsigned short  *Colours_2 = new unsigned short [minims_counter_2];
	int colorstep_2 = 255 / minims_counter_2;
	for (int ii = 0; ii <= minims_counter_2; ii++){
		Colours_2[ii] = (unsigned short) (colorstep_2*ii);
	}
	for (int i = 0; i <= minims_counter_2; ++i){
		f<<"Цвет номер " + to_string(i) + " яркости " + to_string(Colours_2[i])<<endl;
	}
	minims_counter_2++; //ибо там нужно на цвет больше, чем есть минимумов
	//save the second histogramm
	histWidth += 10;
	histHeight += 40;
	//сохранение всей гистограммы
	// hist_image.write(path + "/histogramm " + info + " " + to_string(bi) + ".jpg"); // hist_image.write(path + "/histogramm " + to_string(bi) + ".jpg");
	//сохранение половинки гистограммы - то что надо
	hist_image2.crop(Geometry(histWidth/2+10, histHeight, histWidth/2-5, 0));
	hist_image2.write(path + "/histogramm2 _ " + info + " " + to_string(bi) + ".jpg"); // hist_image.write(path + "/histogramm half " + to_string(bi) + ".jpg");
	...
	for(int i = 0; i < height; i++) {
		...
		delete [] pi_2[i];
		delete [] PI_2[i];
		delete [] pi_average_2[i];
		delete [] channel_R_[i]; //Destructing the model of an image
		delete [] channel_R_2[i]; //Destructing the model of an image
	}	
	delete [] pi;
	delete [] PI;
	delete [] pi_average;
	...
	delete [] minims;
	delete [] Colours;
	//more stuff - the pi-sensetive part
	delete [] points_2;
	delete [] vicinities_2;
	delete [] point_index_2;
	delete [] minims_2;
	delete [] Colours_2;
	//destruct channel
	return channel_R_S;
